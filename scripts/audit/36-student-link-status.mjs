 ...
 *                     open place) AND it has a real open (course×year) slot. A בתהליך
 *                     org (even with places+desc) is HIDDEN; a green-but-full org is
 *                     HIDDEN; a green org on ANOTHER course is HIDDEN.
 *
...
const N = {
  inproc: `SLבתהליך ${ts}`, manual: `SLמאושר-ידני ${ts}`, auto: `SLאוטו-ירוק ${ts}`,
  full: `SLמלא-ירוק ${ts}`, other: `SLקורס-אחר ${ts}`,
};
...
try {
  ...
  const emps = [
    base(`${STU_ID}-e-inproc`, N.inproc, { contactStatus: 'in_process', approvalStatus: 'approved', courseIds: [courseId], vacancySlots: [slot(`${STU_ID}-inproc-s1`, courseId, 'available')] }),
...
audit.log(`LINK: inproc=${seen.inproc} manual=${seen.manual} auto=${seen.auto} full=${seen.full} other=${seen.other}`);
audit.recordCell({
  id: 'LINK-green-only', tableRef: 'OrganizationsPage active filter',
  expected: 'manual-מאושר + auto-green PRESENT; בתהליך + green-but-full + other-course ABSENT',
  observed: `manualPresent=${seen.manual}, autoPresent=${seen.auto}, inprocPresent=${seen.inproc}, fullPresent=${seen.full}, otherPresent=${seen.other}`,
  pass: seen.manual && seen.auto && !seen.inproc && !seen.full && !seen.other,
});

try {
  const data = await loadData();
  const empIds = new Set([`${STU_ID}-e-inproc`, `${STU_ID}-e-manual`, `${STU_ID}-e-auto`, `${STU_ID}-e-full`, `${STU_ID}-e-other`]);
